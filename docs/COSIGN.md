...
```
docker/bake-action  →  image-name (extract digests)  →  trivy  →  cosign-installer (pinned)  →  cosign action (sign + verify)
```
...
- OCI Referrers API attestations (Trivy, Syft outputs).

These are planned follow-ups — the building blocks already exist in CI
(Trivy scan output, bake metadata, OIDC), but wiring them through
...
### 5.2 Sigstore TUF root rotation (handled by 5.1 in practice)

Sigstore rotates the Fulcio root and Rekor public key on a schedule defined
by the upstream TUF metadata. Cosign embeds the current root in the binary,
so step 5.1 inherits any rotation. If an out-of-band manual rotation is
announced, refresh the local trust root:
...
In CI we simply pin a cosign release that already contains the new root.

### 5.3 OIDC issuer / Fulcio endpoint change
...
### 5.5 Compromise / incident response
...
### 5.6 What does *not* need maintenance
