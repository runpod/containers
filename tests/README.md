...
| 4   | **JupyterLab in-pod check** (only when `test_jupyter: true`) — see [Jupyter check](#jupyter-check-opt-in-via-manifest-test_jupyter-true). SSH in, wait for `:8888` to bind, `jupyter server list`, `curl /api/status` with token              | `FAIL` (`start.sh` didn't bring up Jupyter — usually wrong python interpreter)                                                                                                                                   |
...
| `exclude_instances`  | fnmatch-style patterns (case-insensitive) subtracted from the candidate list AFTER `instances:` or budget selection. Useful for blocking known-bad host pairings without rewriting the whole list — e.g. `"*Blackwell*"` skips every Blackwell GPU (sm_100 / sm_120 are not in the kernel set of PyTorch ≤ 2.6 wheels).                                                                           |
