...
| `CPU_FLAVORS` | `""` (uses `cpu-default,cpu-2vcpu-8gb,cpu-4vcpu-16gb`) | CPU "instance candidates". runpodctl 2.3.0 can't pick a CPU flavor by name, but it accepts `--vcpu` / `--mem` minimums that steer the scheduler into different flavor pools. Each label here becomes one candidate iterated by the same per-instance retry loop GPU groups use, so when the cheapest pool is saturated the larger asks often still have free capacity. Format: `label:vcpu:mem,label:vcpu:mem,...`. Use `vcpu=0,mem=0` for "let RunPod pick freely" (matches legacy single-sentinel behaviour). Malformed entries are silently dropped; an empty/all-broken value falls back to the default 3-flavor list. |
...
| only the `base_cpu` group says `no capacity` while GPU groups pass | cheapest CPU flavor pool is saturated in your DC | extend `CPU_FLAVORS` with beefier (vcpu, mem) asks — `CPU_FLAVORS="cpu-default:0:0,cpu-4vcpu-16gb:4:16,cpu-8vcpu-32gb:8:32"` adds two larger fallbacks that often have free machines when the cheapest tier is full |
